<template>
    <section class="productdetail largsec">
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-sm-9 clearfix colprdgal"  :class="{ 'hasoneimg': ad_details.image_count == 1, 'ifcolleft':ad_details.type == 2}" id="hasptdzoomer">
					<div class="col-sm-91 requestDetails ifcolleft1" v-if="ad_details.type == 2" style="padding-left:15px;">
						
						<div class="panel-body form-group">
							<div class="row">
								<div class="col-md-4">
									<div class="prodct_ico" style="background-color:#6d67e5; position:relative;">
										<div class="vcenter">
								...
								<div class="col-md-8">
									<div class="productDinfo">
										<h1 class="prDname">{{ad_details.title}}</h1>
										...
										<h4 class="prdPrice">{{ad_details.price}}€</h4>
										<div class="clearfix"></div>
										
										<div class="clearfix" v-if="user_id != ad_details.user_id*1">
											<!-- <a href="javascript:void(0);" @click="contactCount(ad_details.id, 2)" class="btn btn-default btnprD chatbtn"> -->
											<a href="javascript:void(0);" @click="sendMessage(ad_details)" class="btn btn-default btnprD chatbtn">
												<span><img src="assets/images/ico_bubble.png" class="img-responsive"></span>
												Envoyer un message
												</a>
										...
										<p class="smparaprd paralocatin"><span class="paricon"><img src="assets/images/location.png" class="img-responsive"></span>{{ad_details.address}}</p>

										<p class="smparaprd"><span class="paricon"><img src="assets/images/clock.png" class="img-responsive"></span>{{ad_details.from_date | moment("MMMM Do YY")}} to {{ad_details.to_date | moment("MMMM Do YY")}} from {{ad_details.from_time}}h to {{ad_details.to_time}}h.</p>
										<p class="smparaprd">
											<span class="paricon"><img src="assets/images/briefcase.png" class="img-responsive"></span>{{ad_details.user_type == 2 ? 'Pro' : ''}} Services <strong>{{ad_details.user_name}}</strong> 
											number: <span v-if="ad_details.hide_phone == 0">{{ad_details.contact_phone}}</span>
											<span v-else>Phone number is confidencial</span>
										</p>
										<div class="prDmap hidden">
											<div id="map"></div>
										</div>
										
										<p class="smparaprd" v-if="ad_details.type == 2"><span class="paricon"><img src="assets/images/calender.png" class="img-responsive"></span>Posted on {{ad_details.updated_at | moment("MMMM Do YYYY, h:mm a")}}</p>
									</div>	
						...
						<div id="carousel-example-generic"  v-if="ad_details.type == 1" class="carousel slide" data-ride="carousel">
								
								<div class="carousel-inner" role="listbox">
									<div class="item"  v-bind:class="[index==0 ? 'active' : '']"  v-for="(photo, index) in images.large_size" :key="index">
										<div class="imgbig">
											<figure>
												<img v-img:my-group :src="photo.url" class="img-responsive btn-block" alt="...">
											</figure>
										</div>							      
									</div>
								...
								<a class="left carousel-control" href="#carousel-example-generic" role="button" data-slide="prev">
									<div class="mkcenter"><span class="fa fa-angle-left" aria-hidden="true"></span>
									<span class="sr-only">Previous</span></div>
								</a>
								<a class="right carousel-control" href="#carousel-example-generic" role="button" data-slide="next">
									<div class="mkcenter"><span class="fa fa-angle-right" aria-hidden="true"></span>
									<span class="sr-only">Next</span></div>
								</a>
						</div>

							<div class="mapproduct">
								<div id="map"></div>
							</div>
				...
				<div :class="{ 'col-sm-9 ifrightcol': ad_details.type == 2, 'col-sm-3 colprdinfo': ad_details.type == 1}">
					<div class="productDinfo">
						<h1 class="prDname">{{ad_details.title}}</h1>
						<div class="dropdown dropcs pull-right" v-if="user_id == ad_details.user_id*1">
							<a id="dLabel" data-target="#" href="http://example.com" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
					...
					<div class="callbacksec">
						<h4>Too Busy to call?</h4>
						<div class="row">
							<div class="form-group col-sm-6">
								<input type="text" class="form-control" placeholder="Name" v-model="name">
							</div>
							<div class="form-group col-sm-6">
								<input type="number" class="form-control" placeholder="Phone Number" v-model="phone_number">
							</div>
							<div class="form-group col-sm-12">
								<textarea class="form-control" cols="" rows="3" v-model="details" placeholder="Wtite some details"></textarea>
							</div>
						</div>
						<input id="messagesent" type="button" class="btn btn-default btnprD btn-block callbtn" @click="sendUserQuery" value="Request a callback" disabled>
						<p>Your details will be sent only to this car seller and no one else.</p>
...
export default {
    ...
    methods: {
		sendUserQuery(){
				HTTP.post(`customer-ad-query?api_token=${TOKEN}`, {product_id: this.ad_details.id,
				product_owner_id:this.ad_details.user_id,name:this.name,phone_number:this.phone_number,details:this.details
				})
                .then(response => {
					this.name="";
					this.phone_number="";
		...
		sendMessage($data) {
			// console.log($data)
			this.contactCount($data.id, 2);
			this.$router.push({ path: 'message', query: { message_id: $data.user_id*1 }})
		},
	...
	updated(){
		...
		if(this.name!="" && this.phone_number!="" && this.details!=""){
			
			$("#messagesent").prop('disabled', false);	
		}
		else{
				$("#messagesent").prop('disabled', true);	
		}
	}
}
</script>
